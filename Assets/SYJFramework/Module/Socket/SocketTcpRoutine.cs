...
namespace SYJFramework
{
    ...
    /// <summary>
    /// SocketTcp访问器
    /// 发送数据和接收数据的加密解密由使用者主导，框架只提供实现案例
    /// </summary>
    public class SocketTcpRoutine
    {
            ...
            while (true)
            {
                if (m_ReceiveCount <= GameEntry.Socket.MaxReceiveCount)
                {
                    ...
                    lock (m_ReceiveQueue)
                    {
                        ...
                        {
                            //得到队列中的数据包
                            byte[] bufferNew = m_ReceiveQueue.Dequeue();

                            //处理数据
                            if (OnReceiveDataCallBack != null)
                            {
                                OnReceiveDataCallBack(bufferNew);
                            }
                        }
                        else
                        {
        ...
        /// <returns></returns>
        private byte[] EncryptMsg(byte[] data)
        {
            byte[] retBuffer = null;
        ...
        private void DecryptMsg(byte[] buffer)
        {
            //异或之后的数组
        ...
        {
            //得到封装后的数据包
            //byte[] sendBuffer = EncryptMsg(buffer);
